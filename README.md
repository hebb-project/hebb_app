...
> Only Postgres runs in Docker; the core and web server run on your host. The core
> defaults to that Postgres — `cp .env.example .env` only to override the DB or ports.
> `task dev:down` stops the Postgres container.
